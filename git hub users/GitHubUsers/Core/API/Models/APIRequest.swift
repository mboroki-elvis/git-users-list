...
protocol APIRequest: Encodable {
    ...
    ///
    func response(environment: AppEnvironment) async throws -> ResponseType?
}
...
extension APIRequest {
    ...
    ///
    func doRequest(environment: AppEnvironment) async throws -> ResponseType? {
        if let response = try await response(environment: environment) {
            return response
        }
        return try await doLiveRequest(environment: environment)
    }

    func doLiveRequest(environment: AppEnvironment) async throws -> ResponseType {
        var requestHeaders = environment.headers
        for (key, value) in headers {
            requestHeaders[key] = value
        }
        let jsonData = try JSONEncoder().encode(self)
        let result = await environment.client.request(
            endpoint: endpoint,
            method: method,
